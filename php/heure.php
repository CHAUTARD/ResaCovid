...
// Exemple : 20256 -> 256 éme jours de 2020
$YearNumSelect = date_format( $date, 'yz');
$tpl->assign('dayofyear', $YearNumSelect);
...
foreach($result as $r)
{
    ...
    $horaire[$i] = sprintf("%s : %dh%02d - %dh%02d  (%d/%d)", $r['Salle'], substr($r['Heure_Debut'], 0, 2), substr($r['Heure_Debut'], 3, 2), substr($r['Heure_Fin'], 0, 2), substr($r['Heure_Fin'], 3, 2), $resultN['NbrJ'], $r['Nbr_Place']);
    ...
    $database->bind(':iDate', $YearNumSelect);
    ...
    // Plus de places         
    if( count($resultJoueur) >= $nbrPlace || $dejaInscrit)
    {
        $isLibre[$i] = 'Non';
    }
    else
    {
        ...
        $jourJ = ( $YearNumNow == $YearNumSelect);
