...
$database->query("SELECT l.Prenom as Prenom, l.Nom as Nom FROM `res_reservations` r LEFT JOIN `res_licenciers` l USING(`id_licencier`) WHERE `id_creneau` = :id_creneau AND `iDate` = :iDate AND r.`Ouvreur` = 'Non'");
$database->bind('id_creneau', $_GET['id_creneau']);
$database->bind('iDate', $_GET['iDate']);
